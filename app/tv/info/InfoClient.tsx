...
export default function InfoClient() {
    ...
    useEffect(() => {
        const onKey = (e: KeyboardEvent) => {
            ...
            if (isEnter) {
                e.preventDefault();
                openRoute(focus === 0 ? "/tv/menus" : "/tv/timetables");
            }
        };
        ...
        >
            ...
            <div style={{ position: "relative", height: "100%", padding: 44, boxSizing: "border-box" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 24 }}>
                    ...
                    <div style={{ fontSize: 18, color: "white", opacity: 0.9, textShadow: "0 4px 20px rgba(0,0,0,0.8)" }}>
                        {t.hint}
                    </div>
                </div>

                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 18, height: "calc(100% - 80px)" }}>
                    {/* LEFT */}
                    <div
                        style={{
                            borderRadius: 24,
                    ...
                    >
                        {t.blocks.map((b) => {
                            const isWifi = b.title.includes("Wi") || b.title.includes("WLAN") || b.title.includes("Wi-Fi");
                            ...
                            return (
                                ...
                                >
                                    <div style={{ flex: 1 }}>
                                        <div style={{ fontSize: 22, fontWeight: 800, marginBottom: 6, color: "white" }}>{b.title}</div>
                                        {b.lines.map((ln, i) => (
                                            <div key={i} style={{ fontSize: 20, color: "rgba(255,255,255,0.9)", lineHeight: 1.25, letterSpacing: "0.02em" }}>
                                                {ln}
                                            </div>
                                        ))}
                                    </div>

                                    {isWifi && <QrCode value={wifiQr} size={150} />}
                                </div>
                            );
                        })}
                    </div>
