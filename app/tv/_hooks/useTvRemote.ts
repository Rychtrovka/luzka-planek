...
const KEY_BACK = new Set(["Escape", "Backspace", "BrowserBack", "Back"]);
...
export function useTvRemote({
  ...
  useEffect(() => {
    ...
    // ✅ Android ovladač BACK posíláme z native jako custom event
    const onNativeBack = (e: Event) => {
      e.preventDefault?.();
      // žádný stopPropagation tady není potřeba
      handleBack();
    };

    window.addEventListener("keydown", onKeyDown, true);
    window.addEventListener("rychtrovka:back", onNativeBack as any, true);

    return () => {
      window.removeEventListener("keydown", onKeyDown, true);
      window.removeEventListener("rychtrovka:back", onNativeBack as any, true);
    };
  }, [
    screen,
    setScreen,
