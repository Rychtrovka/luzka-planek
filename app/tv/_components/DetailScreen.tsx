...
export default function DetailScreen({ t, styles, currentTileId }: Props) {
    ...
    useEffect(() => {
        ...
        QRCode.toDataURL(qrPayload, {
            ...
            .then(url => {
                if (alive) setQrDataUrl(url) // OPRAVENO: nyní odpovídá useState
            })
            .catch((err) => {
    ...
    const QuickLink = ({ icon, title, sub, path }: { icon: string, title: string, sub: string, path: string }) => (
        <button
            type="button"
    ...
    return (
        <div style={{ ...styles.page, overflowY: "auto", paddingBottom: 40 }}>
            ...
            {wifi?.ssid && (
                <div style={{ display: "flex", gap: 28, marginBottom: 22 }}>
                    <div style={cardStyles.glass}>
                        <div style={{ fontSize: 30, fontWeight: 850, marginBottom: 10 }}>📶 {wifi.label || "Wi-Fi"}</div>
                        <div style={{ fontSize: 22, lineHeight: 1.5 }}>
                            <div><b>SSID:</b> {wifi.ssid}</div>
                            <div><b>Heslo:</b> {wifi.password || "(bez hesla)"}</div>
                        </div>
                        {wifi.qrHint && <div style={{ marginTop: 12, fontSize: 18, opacity: 0.85 }}>{wifi.qrHint}</div>}
                    </div>
            ...
            {/* RYCHLÉ ODKAZY */}
            <div style={{ fontSize: 20, opacity: 0.85, marginBottom: 12 }}>{info.quickLinksTitle || "Rychlé odkazy"}</div>
